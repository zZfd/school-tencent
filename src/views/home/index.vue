...
<template>
  <div class="home-container">
    <div class="h-nav-sticky">
      <row class="h-nav">
        <van-col span="8" class="h-nav-i" :class="active?'h-nav-i--ative':''" @click="clickPopup">{{ currentSubject }}</van-col>
        <van-col span="8" class="h-nav-i" :class="timeUp?'h-nav-i--ative':''" @click="clickSort('time')">时间</van-col>
        <van-col span="8" class="h-nav-i" :class="priceUp?'h-nav-i--ative':''" @click="clickSort('price')">价格</van-col>
      </row>
    </div>

    <tree-select v-show="popupSubject" class="home-popup-s" height="" :items="catgories" :active-id.sync="activeId" :main-active-index.sync="activeIndex" @click-item="clickSubject" />
    <div class="h-s-ul">
      <pull-refresh v-model="refreshLoading" success-text="刷新成功" @refresh="onRefresh">
        <div v-for="(item,index) in subjects" :key="index" class="h-s-item" @click="$router.push('/subject/detail')">
...
<script>
import { Col, Row, TreeSelect, PullRefresh } from 'vant'
